...
for nodo = 1:numel(path)-1
    distancia = distancia + G.Edges.length(findedge(G, path(nodo), path(nodo + 1)));
...
for nodo = 1:numel(path2)-1
    distancia2 = distancia2 + G.Edges.length(findedge(G, path2(nodo), path2(nodo + 1)));
...
for nodo = 1:numel(path3)-1
    distancia3 = distancia3 + G.Edges.length(findedge(G, path3(nodo), path3(nodo + 1)));
